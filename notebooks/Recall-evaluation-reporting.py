...
df_combined_selected = pd.read_csv(
    f"output/forward/{report_name}.csv",
    usecols=[
        "scenario_name",
        "n_recall_diff",
...
df_combined_selected["n_recall_diff"] = df_combined_selected["n_recall_diff"]
df_combined_selected["split_before_merge"] = df_combined_selected[
    "split_before_merge"
].astype(int)
...
plt.savefig(
    "output/forward/figures/forward-mlr-ols-coefficients-significant.svg",
    bbox_inches="tight",
)
plt.savefig(
    "output/forward/figures/forward-mlr-ols-coefficients-significant.eps",
    bbox_inches="tight",
)
...
for i, y in enumerate(y_cols):
    for j, x in enumerate(x_cols):
        ...
        try:
            ...
            df_plot.boxplot(column=y, by=x, ax=ax, grid=False)
...
plt.tight_layout()
# plt.savefig("output/forward/figures/boxplot-parameters-avg_out_degree.eps")
...
for i, y in enumerate(y_cols):
    for j, x in enumerate(x_cols):
        ...
        for cat in categories:
            ...
            # err_low.append(np.percentile(subset, (100 - ci) / 2))
            # err_high.append(np.percentile(subset,  100 - (100 - ci) / 2))
...
plt.suptitle("")
plt.tight_layout()
plt.savefig("output/forward/figures/ci-parameters-n_recall_diff.eps")
